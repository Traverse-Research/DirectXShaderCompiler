...
bool LowerStaticGlobalIntoAlloca::lowerStaticGlobalIntoAlloca(
  ...
  // Iteration order of this map is observable: rewriting a use below appends
  // the replacement instructions to AI's use list, and PatchDebugInfo() walks
  // AI->users() to decide the order in which it creates a DILocalVariable per
  // inlined DISubprogram. A DenseMap keyed on Instruction* iterates in pointer
  // (hence heap-address) order, which made that debug info come out permuted
  // between runs of the same compilation. Keep insertion order.
  MapVector<Instruction *, Value *> InstUserMap;
